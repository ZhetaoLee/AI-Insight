...
- **"Weekly time saved"** — its old footer referenced `avg_weekly_hours_saved.denominator`; the distribution-panel fix replaced that with a top-bucket fact, and the later average-removal fix removed the midpoint-derived metric entirely.
